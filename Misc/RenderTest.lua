local master = {objects = {}, x = 0, y = 0, w = 160, h = 50, allocatedBuffer}
    ...
    perObject = function()
        ...
        for i, v in pairs(objects) do
            if v.changed then
                ...
                drawFrom(1, bObj)
                screenUpdate()
                setDrawLimit(v.rawx, v.rawy, v.rawx + v.w - 1, v.rawy + v.h - 1)
                v:draw()
                colidesWith(v)
                v:updateBuffer()
                if debug then debugInfo[1] = debugInfo[1] + 1 end
...
function setFullDrawLimit()
    setDrawLimit(master.x + 1, master.y + 1, master.w, master.h)
end
...
local raws = {rawx=true,rawy=true,raww = true, rawh = true}
